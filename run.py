    ...
    async with Browser(headless=headless, profile=True) as ctx:
        for site in targets:
            ...
            if STOP.exists():
                ...
                break
            # GIA FATTO per QUESTO account (sito+account in keys.txt) -> non rifare (vale anche se only)
            ...
            # Anche se chiesto esplicito (only): segnala il muro invece di sbattere contro.
